...
export function dragula (initialContainers, options) {
	if (options == null && Array.isArray(initialContainers) === false) {
	...
	function movements (remove) {
		var op = remove ? 'remove' : 'add';
		if (op == 'add') {
			documentElement.addEventListener('selectstart', preventGrabbed);
			documentElement.addEventListener('click', preventGrabbed);
		} else {
			documentElement.removeEventListener('selectstart', preventGrabbed);
			documentElement.removeEventListener('click', preventGrabbed);
		}
	}
	...
	function cancel (revert) {
		...
		var reverts = revert != null ? revert : o.revertOnSpill;
		var item = _copy || _item;
		var parent = getParent(item);
	...
	function renderMirrorImage () {
		...
		_mirror.classList.replace('gu-transit', 'gu-mirror');

...
function DistanceMarker(props) {
	...
	return (
		<Fragment>
			<text class="distanceMarker" x={`${props.x}%`} y={`${y - (props.up ? -0.8 : 0.8)}%`} font-size="10px" text-anchor="middle" dominant-baseline={props.up ? "hanging" : "auto"} fill="var(--brand-color)">{`${props.d}m`}</text>
			<line x1={`${props.x}%`} y1={`${y}%`} x2={`${props.x}%`} y2={`${y + (props.up ? -2.5 : 2.5)}%`} stroke="var(--brand-color)" />
		</Fragment>
	);
}

function SectionText(props) {
	return <text class="sectionText" x="50%" y="50%" height="40%" width="100%" fill="var(--brand-color)"><Text id={`racetrack${props.w < 0.085 ? '.short' : ''}.${props.id}`} fields={props.fields} /></text>;
}
...
export function RaceTrack(props) {
	...
	const almostEverything = useMemo(function () {
		...
		const slopes = full.reduce((elems,s,i) => {
			...
			if (s.slope == 0) {
				elems.push(<rect x={`${s.start / course.distance * 100}%`} y={`${lastEndHeight * 0.262}%`} width={`${s.length / course.distance * 100}%`} height="26.2%" fill="var(--brand-color-light)" />);
			} else {
				elems.push(
					<svg class={`hillArea ${s.slope < 0 ? 'downhill' : 'uphill'}`} x={`${s.start / course.distance * 100}%`} y="0" width={`${s.length / course.distance * 100}%`} height="26.2%" viewBox="0 0 100 100" preserveAspectRatio="none">
						<polygon points={`0,${lastEndHeight} 0,100 100,100 100,${thisEndHeight}`} fill="var(--brand-color-light)" />
					</svg>
				);
			}
			return elems;
		...
		return (
			<Fragment>
				{slopes}
				<rect x="0" y="26.2%" width="100%" height="1.8%" fill="var(--brand-color-dark)" />
				<svg class="sectionsBg" x="0" y="28%" width="100%" height="18%">
					<rect x="0" y="0" height="90%" width="100%" fill="var(--border-color-light)" />
					<rect x="0" y="90%" height="10%" width="100%" fill="var(--border-color)" />
				</svg>
				{course.slopes.map(s =>
					<svg class="slope" x={`${s.start / course.distance * 100}%`} y="28%" width={`${s.length / course.distance * 100}%`} height="18%">
						<rect x="0" y="0" height="90%" width="100%" fill={s.slope > 0 ? 'hsl(45, 70%, 80%)' : 'hsl(170, 70%, 80%)'} />
						<rect x="0" y="90%" height="10%" width="100%" fill={s.slope > 0 ? 'hsl(45, 70%, 60%)' : 'hsl(170, 70%, 60%)'} />
				...
				{course.straights.map((s,i) =>
					<svg class="straight" x={`${s.start / course.distance * 100}%`} y="46%" width={`${(s.end - s.start) / course.distance * 100}%`} height="18%">
						<rect x="0" y="0" height="90%" width="100%" fill={'hsl(210, 100%, 80%)'} />
						<rect x="0" y="90%" height="10%" width="100%" fill={'hsl(210, 100%, 60%)'} />
				...
				{course.corners.map((c,i) =>
					<svg class="corner" x={`${c.start / course.distance * 100}%`} y="46%" width={`${c.length / course.distance * 100}%`} height="18%">
						<rect x="0" y="0" height="90%" width="100%" fill={'hsl(30, 100%, 80%)'} />
						<rect x="0" y="90%" height="10%" width="100%" fill={'hsl(30, 100%, 60%)'} />
				...
				<svg class="phase phase0" x="0" y="64%" width="16.67%" height="18%">
					<rect x="0" y="0" height="90%" width="100%" fill="hsl(150, 100%, 40%)" />
					<rect x="0" y="90%" height="10%" width="100%" fill="hsl(150, 100%, 20%)" />
					<SectionText id="phase0" w={0.1667} />
				</svg>
				<svg class="phase phase1" x="16.67%" y="64%" width="50%" height="18%">
					<rect x="0" y="0" height="90%" width="100%" fill="hsl(60, 100%, 60%)" />
					<rect x="0" y="90%" height="10%" width="100%" fill="hsl(60, 100%, 40%)" />
					<SectionText id="phase1" w={0.5} />
				</svg>
				<svg class="phase phase2" x="66.67%" y="64%" width="16.67%" height="18%">
					<rect x="0" y="0" height="90%" width="100%" fill="hsl(330, 100%, 70%)" />
					<rect x="0" y="90%" height="10%" width="100%" fill="hsl(330, 100%, 50%)" />
					<SectionText id="phase2" w={0.1667} />
				</svg>
				<svg class="phase phase3" x="83.33%" y="64%" width="16.67%" height="18%">
					<rect x="0" y="0" height="90%" width="100%" fill="hsl(330, 100%, 60%)" />
					<rect x="0" y="90%" height="10%" width="100%" fill="hsl(330, 100%, 40%)" />
				...
				<rect x="0" y="82%" height="18%" width="100%" fill="var(--border-color-light)" />
				{Array.from({length: 25}, (_,i) => i).map(i => <line x1={`${i / 24 * 100}%`} y1="96%" x2={`${i / 24 * 100}%`} y2="100%" stroke="var(--border-color)" stroke-width={i == 0 || i == 24 ? "4" : "2"} />)}
				{Array.from({length: 24}, (_,i) => i + 1).map(i => <text x={`${(1/48 + (i-1)/24) * 100}%`} y="91%" font-size="10px" text-anchor="middle" dominant-baseline="central" fill="var(--text-color)">{i}</text>)}
				<rect x="0" y="98.2%" height="1.8%" width="100%" fill="var(--border-color)" />
			</Fragment>
		);
	}, [props.courseid]);
	...
	return (
		<IntlProvider definition={lang == 'ja' ? STRINGS_ja : STRINGS_en}>
			<div class="racetrackWrapper modern" style={`width:${props.width + xOffset + xExtra}px`}>
				{trackNameHeader}
				<svg version="1.1" width={props.width + xOffset + xExtra} height={props.height + yOffset + yExtra} xmlns="http://www.w3.org/2000/svg" class="racetrackView" data-courseid={props.courseid} onMouseMove={doMouseMove} onMouseLeave={doMouseLeave}>
					<svg x={props.xOffset} y={props.yOffset} width={props.width} height={props.height}>
						{almostEverything}
						{regions}
						<line class="mouseoverLine" x1="-5" y1="0" x2="-5" y2="100%" stroke="var(--brand-color)" stroke-width="2" />
						<text class="mouseoverText" x="-5" y="-5" fill="var(--brand-color)"></text>
					</svg>
					{props.children}
				</svg>
			</div>
